          ...
          image.width.times do |j|
            ...
            start_i.upto(start_i + size - 1) do |ii|
              start_j.upto(start_j + size - 1) do |jj|
                array_pixels << {
                  get_gr(
                    image.view[ii][jj].red,
                    image.view[ii][jj].green,
                    image.view[ii][jj].blue
                  ) => [image.view[ii][jj].red,
                    image.view[ii][jj].red, image.view[ii][jj].blue]
            ...
            image.view[i][j].red = result.first[1][0]
            image.view[i][j].green = result.first[1][1]
            image.view[i][j].blue = result.first[1][2]
